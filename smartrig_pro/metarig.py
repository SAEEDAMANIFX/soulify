...
def _fit_core(mo, props, J, h, ground, yc):
    ...
    # ---- no finger data at all -> REMOVE the template finger bones ----
    # (leaving them keeps Rigify's default fingers floating in mid air far
    # from the hand - same policy as the face bones removal below)
    if not hand_m:
        _gone = 0
        for b in list(eb):
            if b.name.startswith(("f_index", "f_middle", "f_ring", "f_pinky",
                                  "thumb.", "palm.")):
                try:
                    eb.remove(b); _gone += 1
                except Exception:
                    pass
        if _gone:
            print("Soulify: no fingers placed - removed %d template finger "
                  "bones (add fingers via the Hands step, then rebuild)" % _gone)
    ...
    def execute(self, context):
        ...
        try:
            ...
            if rg is not None and _kn.kandura_object(context) is not None:
                _acts = _kn.ensure_kandura_bind(rg, p)
                if _acts:

...
def _facial_autodetect(props, context):
    ...
    # candidates must live in the CURRENT VIEW LAYER (excluded collections,
    # other scenes and orphan asset copies do NOT qualify - binding an
    # idle-pose duplicate onto the t-pose rig corrupted a real character)
    # and sit near the target mesh's head.
    _top = _hgt = None
    if mesh is not None:
        try:
            import numpy as _np
            _c = _np.array([(mesh.matrix_world @ v.co).z
                            for v in mesh.data.vertices[::23]])
            _top = float(_c.max())
            _hgt = max(_top - float(_c.min()), 1e-6)
        except Exception:
            pass
    ...
    def near_head(ob):
        ...
        try:
            c = sum((ob.matrix_world @ Vector(b) for b in ob.bound_box),
                    Vector()) / 8.0
            return (_top - 0.4 * _hgt) < float(c.z) < (_top + 0.1 * _hgt)
        except Exception:
            return True

    eyes = []
    for ob in context.view_layer.objects:
        if ob.type != 'MESH' or ob in taken or not near_head(ob):
            continue
        nm = ob.name.lower()
        ...
        elif "hair" in nm and free("skin_hair"):
            props.skin_hair = ob; found += 1
    for ob in eyes:
        nm = ob.name.lower()
...
def bind_facial_features(props, context, rig):
    ...
    slots = (("skin_eye_l", "eye"), ("skin_eye_r", "eye"),
             ...
             ("skin_tongue", "tongue"), ("skin_hair", "hair"))
    n = 0
...
def unbind_mesh(props, context):
    ...
    for slot in ("skin_hair", "skin_eye_l", "skin_eye_r", "skin_teeth_up",
                 "skin_teeth_low", "skin_tongue"):
        ob = getattr(props, slot, None)

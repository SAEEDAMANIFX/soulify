...
def read_rest_coords(obj):
    """(N,3) world coords from the BASE (undeformed) mesh data - ignores the
    Armature and any deform modifiers. Use this for fitting bones to a mesh that
    may already be rigged/posed (e.g. rebuilding the skirt after Generate): the
    evaluated mesh would be DEFORMED by the current pose/jiggle, so bones would be
    placed on the momentary shape and jump around. The rest data is stable."""
    me = obj.data
    n = len(me.vertices)
    c = np.empty(n * 3, dtype=np.float64)
    me.vertices.foreach_get("co", c)
    c = c.reshape(n, 3)
    mw = np.array(obj.matrix_world)
    return (np.column_stack([c, np.ones(n)]) @ mw.T)[:, :3]

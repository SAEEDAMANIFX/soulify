...
def _draw_glow(region, rv3d):
    ...
    for name in markers.all_marker_names():
        ...
        if not p:
            continue
        col = GLOW[_role(name)]
        s = (1.4 if _selected(o) else 1.0) * m
        ...
        shader.bind(); shader.uniform_float("color", (1, 1, 1, 0.95)); b.draw(shader)
    gpu.state.blend_set('NONE')
...
def _draw_fingers(region, rv3d):
    ...
    for part, side in pairs:
        for fn, chain in fingers_manual.list_fingers(part, side).items():
            ...
            m = _marker_mult()
            for (px, py) in pts:
                for rr, aa in ((20 * m, 0.12), (13 * m, 0.26), (8 * m, 0.60)):
                    b = batch_for_shader(shader, 'TRI_FAN', {"pos": _circle(px, py, rr)})
                    shader.bind(); shader.uniform_float("color", (col[0], col[1], col[2], aa)); b.draw(shader)
                b = batch_for_shader(shader, 'TRI_FAN', {"pos": _circle(px, py, 3.5 * m)})
                shader.bind(); shader.uniform_float("color", (1, 1, 1, 0.95)); b.draw(shader)
    gpu.state.blend_set('NONE')

...
def ai_root():
    ...
    # last-good path remembered across .blend files (per-scene props die with
    # every new file - that silently disabled the finger AI on new scenes)
    try:
        _memo = os.path.join(bpy.utils.user_resource('CONFIG'),
                             "soulify_ai_path.txt")
        if os.path.isfile(_memo):
            cands.append(open(_memo, encoding="utf-8").read().strip())
    except Exception:
        pass
    ...
    for c in cands:
        if c and os.path.exists(os.path.join(c, "info.dat")):
            try:
                open(os.path.join(bpy.utils.user_resource('CONFIG'),
                     "soulify_ai_path.txt"), "w", encoding="utf-8").write(c)
            except Exception:
                pass
            return c
    return None

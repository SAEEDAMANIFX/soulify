...
class SMARTRIG_OT_finger_place(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if self.mesh is None or self.mesh.type != 'MESH':
            ...
            return {'CANCELLED'}
        if p.guide_active:                       # leaving the body-marker guide cleanly
            try:
                markers._guide_finish(context)
            except Exception:
    ...
    def invoke(self, context, event):
        ...
        if p.guide_active:
            try:
                ...
                p.guide_active = False; p.placing = False
...
class SMARTRIG_OT_align_selected(bpy.types.Operator):
    ...
    axis: bpy.props.EnumProperty(
        name="Axis", default='Z',
        items=[('Z', "Z (height)", ""), ('X', "X (side)", ""), ('Y', "Y (depth)", "")])
    ...
    def execute(self, context):
        ...
        avg = sum(o.location[ai] for o in sel) / len(sel)
        for o in sel:
            loc = o.location.copy(); loc[ai] = avg; o.location = loc
        for a in context.window.screen.areas:
            a.tag_redraw()
        return {'FINISHED'}
...
classes = (SMARTRIG_OT_finger_place, SMARTRIG_OT_finger_next, SMARTRIG_OT_finger_resume,
           SMARTRIG_OT_finger_finish, SMARTRIG_OT_finger_remove,

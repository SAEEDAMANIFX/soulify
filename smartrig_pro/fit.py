...
def compute_joints(props):
    ...
    J = {}
    # --- axial chain
    pelvis = Vector((0.0, _volume_center_y(co, 0.0, m_root.z, h), m_root.z))
    neck_base = _torso_center(co, m_neck.z, h, Vector((0.0, pelvis.y, m_neck.z)))
    head_base = Vector((0.0, neck_base.y, neck_base.z + 0.4 * (m_head.z - neck_base.z)))
    J["pelvis"] = pelvis
    J["neck_base"] = neck_base
    J["head_base"] = head_base
    J["head_top"] = Vector((0.0, head_base.y, m_head.z))
...
def orient_bones_pro(arm, J, co, h):
    ...
    for nm in list(eb.keys()):
        if nm == "root" or nm == "neck_01" or nm == "head" or nm.startswith("spine_"):
            ...
            setz(nm, FRONT)
        elif nm.startswith("thigh") or nm.startswith("shin") or nm.startswith("foot"):
            setz(nm, BACK)
        elif nm.startswith("toe"):
            setz(nm, UP)
        elif nm.startswith("f_") or nm.startswith("palm") or nm.startswith("thumb"):
            b = eb.get(nm)
            if b is not None:
                d = (b.tail - b.head)
                if d.length > 1e-6:
                    setz(nm, Y.cross(d.normalized()))
